...
const FRONTEND_VERSION = '3.2.8';
const api = {
  metadata: '/api/metadata',
  formulaEvent: '/api/formula_event',
  formulaBatch: '/api/formula_batch'
};
...
async function postJSON(url, body) {
  ...
  if (!response.ok) {
    ...
    if (Array.isArray(detail)) {
      ...
      message = String(detail || `Request failed with status ${response.status}`);
    }
...
async function init() {
  ...
  if (state.meta?.app_version !== FRONTEND_VERSION) {
    throw new Error(`App version mismatch: frontend ${FRONTEND_VERSION}, backend ${state.meta?.app_version || 'older/unknown'}. Replace both app.py and static/, then restart.`);
  }
